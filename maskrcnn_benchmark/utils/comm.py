...
def get_world_size():
    if not torch.distributed.distributed_c10d.is_initialized():
        return 1
    return torch.distributed.distributed_c10d.get_world_size()


def get_rank():
    if not torch.distributed.distributed_c10d.is_initialized():
        return 0
    return torch.distributed.distributed_c10d.get_rank()


def is_main_process():
    if not torch.distributed.distributed_c10d.is_initialized():
        return True
    return torch.distributed.distributed_c10d.get_rank() == 0
...
def synchronize():
    ...
    if not torch.distributed.distributed_c10d.is_initialized():
        return
    world_size = torch.distributed.distributed_c10d.get_world_size()
    rank = torch.distributed.distributed_c10d.get_rank()
    if world_size == 1:
        return
    ...
    def _send_and_wait(r):
        ...
        torch.distributed.distributed_c10d.broadcast(tensor, r)
        while tensor.item() == 1:
            time.sleep(1)
...
def scatter_gather(data):
    ...
    torch.distributed.distributed_c10d.broadcast(data_to_communicate, 0)
